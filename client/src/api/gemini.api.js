...
export const generateQuestions = async (data) => {
  try {
    // Corrected path from '/generatequestions' to '/generate-questions'
    const response = await axios.post(`${API_URL}/generate-questions`, data, {
      headers: { 'Content-Type': 'application/json' },
      withCredentials: true,
    });
    return response.data; // It's better practice to return response.data
  } catch (error) {
...
export const generateReview = async (data) => {
  try {
    // Corrected path from '/generatereview' to '/generate-review'
