...
import { useJudge0 } from "../../utils/hooks/useJudge0"
import { languages, getLanguageId, getFileExtension } from "../../utils/languageConfig"
...
export default function CodeEditor() {
  const [files, setFiles] = useState<File[]>([{ name: "Untitled", content: "", language: "JavaScript" }])
  const [activeFile, setActiveFile] = useState(0)
  const [input, setInput] = useState("")
  const editorRef = useRef(null)
  const { runCode, output, error, isLoading } = useJudge0()
  ...
  const handleAddFile = () => {
    ...
    setFiles([...files, { name: newFileName, content: "", language: "JavaScript" }])
  ...
  const handleRunCode = () => {
    ...
    runCode(currentFile.content, getLanguageId(currentFile.language), input)
  }
      ...
      <PanelGroup direction="vertical" className="flex-grow">
        <Panel defaultSize={60} className="h-full">
          <Tabs
            ...
            onValueChange={(value) => handleTabChange(Number.parseInt(value))}
            className="flex flex-col h-full"
          >
            <TabsList className="bg-zinc-800 border-b border-zinc-700">
              {files.map((file, index) => (
                <TabsTrigger key={index} value={index.toString()} className="data-[state=active]:bg-zinc-900">
            ...
            {files.map((file, index) => (
              <TabsContent key={index} value={index.toString()} className="flex-1 h-[calc(100vh-240px)]">
                <Editor
                  height="100%"
